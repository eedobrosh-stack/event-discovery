...
# TheSportsDB league id + season come from the unified tournament
# catalog (app/services/tournaments.py). Tournaments with `tsdb_league
# = None` skip the TheSportsDB step entirely — the script falls back to
# the LLM broadcaster map for them. That's the right shape for tennis
# Grand Slams (Wimbledon, Roland Garros, US Open): TheSportsDB serves
# per-match tennis data but our DB rows are the whole multi-day
# tournament, so there's no clean match key.
...
# Broadcaster maps come from the unified tournament catalog at
# app/services/tournaments.py. The local fetch helper just looks
# up the per-country dict for the active tournament; widening the
# coverage (adding entries to the catalog) is a single-file edit.
...
def _llm_channels_for(tournament: str) -> list[dict]:
    ...
    out: list[dict] = []
    for iso2, channels in broadcasters_for(tournament).items():
        for c in channels:
            out.append({
                "channel": c["channel"],
                "country": iso2,
...
def main() -> int:
    ...
    tournaments = (
        [args.tournament] if args.tournament else list(TOURNAMENTS.keys())
    )
    ...
    try:
        ...
        for tour in tournaments:
            league_id, season = tsdb_mapping(tour)
            # tsdb_league=None means "skip TheSportsDB for this
            # tournament" (tennis Grand Slams etc.) — apply the LLM
            # broadcaster map directly. The flow below already handles
            # the empty tsdb_map case by falling through to LLM.
            if league_id and season:
                ...
                log.info("=== %s (LLM-only — no TheSportsDB mapping) ===", tour)
                tsdb_events = []
