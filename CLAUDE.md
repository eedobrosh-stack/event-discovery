...
| `scripts/dedupe_venues.py` | Venue-row deduper — ≥2-of-5 signals (geo / events / phone / url / address), name-based sub-venue (hall) veto, `--merge-pair` override for known cross-language clusters. `--website-pass [--country X]` consolidates same-city rows sharing a website host (ticketing/map/municipal aggregators excluded) whose names also agree, and records every merged spelling in `venue_aliases` so the next scrape re-lands on the survivor. `--site-pass --country Israel` is the looser product rule (same site = same venue unless the names name different cities; shared ticketing/municipal/chain sites key on the page; junk waze/youtube/maps URLs cleared; website-less exact-name twins inherit the site), grouping by real city via `app/services/il_places.py`; also applies SITE_MOVES (a venue's new site), inherits sites for website-less twins by name / alias / identity words, and renames each merged venue to ONE display name (DISPLAY_NAMES overrides) |
| `scripts/rehome_israel_venues.py` | Moves Israeli venues to the City their `physical_city` names (via `il_places`); places without a City row → "Israel - Other"; abroad → its City row abroad or skipped. Adds aliases under the new city, keeps the old-city ones so default-city collectors keep resolving. Ingest now does the same up front (`CollectorRegistry._venue_home_city`) |
| `app/services/venue_display.py` | One venue, one name: API/export show `Venue.name` (not the scraped `events.venue_name`) and the canonical Israeli city; search matches `Venue.name` OR any `venue_aliases` spelling; suggestions give one chip per venue. City aliases (`/api/cities` `aliases`, Hebrew via `il_places`) feed the location autocomplete |
| `app/services/event_conflicts.py` + `scripts/resolve_event_conflicts.py` | Rule A: same title in a city with ONE artist → rows without it get artist + YouTube, rows without types get the siblings' category/format. Rule B: same title (or same artist at same venue / exact time), same day + city, ≤3 h apart, different sources = one show → keep the EARLIEST time, fold fields/types in, move to the cluster's most-established venue. One source at two times/venues = separate shows. Runs at ingest (`find_time_conflict` + the same-venue check), every 6 h (`resolve_event_conflicts_job`, 01/07/13/19:45 UTC) and via the script; supercaly-qa reports it as `event_conflicts` |
